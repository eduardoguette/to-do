...
function formatLocalISO(date = new Date()) {
  const tzOffset = date.getTimezoneOffset() * 60000 // minutos a ms
  const local = new Date(date - tzOffset)
  return local.toISOString().slice(0, -1) // quita la Z
}

const date = formatLocalISO() 
  const idUser = supabase.auth.session()?.user.id
  if (!idUser) return (result = { logged: false })
  result = { ...result, idUser, date, logged: true }
  await supabase.auth.onAuthStateChange(
    (_event, session) => (result = { ...result, session })
  )
  const profile = (await getProfile(idUser)) || null
  if (!profile)
    return (result = { ...result, logged: true, estado: 'noProfile' })
  await downloadImage(profile.avatar_url).then(
    (avatar) => (result = { ...result, avatar, profile })
  )
  return result
}
...
async function getProfile(id) {
  try {
    let { data, error, status } = await supabase
      .from('profiles')
      .select(`location, name, avatar_url, id, biography`)
      .eq('id', id)
      .single()
...
async function uploadAvatar(event) {
  try {
    ...
    const idUser = supabase.auth.session()?.user.id
    const filePath = `${idUser}/${fileName}`
